...
export default{
	...
	mutations:{
		...
		loginSuccess(state,payload){
			...
			state.loading = false;
			state.message = "Salut! Vous etes connecté...";
			state.currentUser = Object.assign({},payload.user,{token:payload.access_token});

			localStorage.setItem("user",JSON.stringify(state.currentUser));
		},
		loginFailed(state,payload){
			state.message="Infos de connexion invalide"
			state.loading = false;
		...
		registerSuccess(state,payload){
			state.authErr = null;
			state.loading = false;
			state.message = "Inscription avec succès, Veuillez procéder à la connexion";
		},
		...
		logout(state){
			state.message = "Vous etes connectét...";
		...
		loadQuizzes(state,payload){
			if(payload.length == 0){
				state.message = "Pas de test disponible."
			}
			else
			{
	...
	actions:{
		...
		PlayQuiz(context,qid){
			...
			.then((response)=>{
				...
				if(response.data.questions.length > 0){
					...
					context.commit("showAlert","Pas de questions disponible....");
				}
			})
		...
		storeQuizQuestions(context,payload){
			context.commit("toggleLoading",true);
			context.commit("showAlert","Test en cours d'enregistrement... Veuillez patienter");
			...
			.then((response)=>{
				...
				if(response.data.quiz == null){
					context.commit("showAlert","Could not save test. Invalid Inputs");
				} else {
					context.commit("showAlert","Quiz "+response.data.quiz+" Ajouter avec succès");
				}
			},(error)=>{
				context.commit("toggleLoading",false);
				context.commit("showAlert","Could not save test. Server error");
		...
		DeleteQuiz(context,payload){
			...
			{
				context.commit("showAlert","Vous ne pouvez pas répondre à cette question!");
				return false;
			}
